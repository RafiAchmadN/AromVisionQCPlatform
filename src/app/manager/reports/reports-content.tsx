  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 bg-brand-50 border-b border-brand-100">
        ...
        <button type="button" onClick={load} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">{t('common.refresh')}</button>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-col">
            ...
            {data.rejection_rate !== undefined && (
              <StatRow label={lang === 'en' ? 'Rejection Rate' : 'Tingkat Penolakan'} value={`${data.rejection_rate}%`}
...
export function ManagerReportsContent() {
  ...
  const PERIODS: { key: Period; label: string }[] = [
    { key: 'daily',   label: t('rpt.daily')   },
    { key: 'weekly',  label: t('rpt.weekly')  },
    { key: 'monthly', label: t('rpt.monthly') },
  ];
